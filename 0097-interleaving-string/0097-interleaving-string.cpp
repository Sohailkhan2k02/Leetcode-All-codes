...
public:
    bool f(int i, int j, int k, string s1,string s2, string s3,int n,int m,vector<vector<int>>&dp){
        ...
        if(i<n && s1[i]==s3[k] && s2[j]!=s3[k]){
            dp[i][j]=f(i+1,j,k+1,s1,s2,s3,n,m,dp);
        }
        else if(j<m && s2[j]==s3[k] && s1[i]!=s3[k]){
            dp[i][j]=f(i,j+1,k+1,s1,s2,s3,n,m,dp);
        }
        else if(i<n && j<m && s1[i]==s3[k] && s2[j]==s3[k]){
            dp[i][j]=f(i+1,j,k+1,s1,s2,s3,n,m,dp)||f(i,j+1,k+1,s1,s2,s3,n,m,dp);
        }
        return dp[i][j];
    }
